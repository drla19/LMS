...
';
 ...
 if (($_GET["selected_div"]!=0) || ($_GET["cmd"]=='imageback')) $content.='visibility: hidden; display: none;';
  

  
 $themes = getdir($themes_dir);
...
$content.='">
 ...
 <input id="selected_t" type="hidden" value="1" />
  ...
  <div id="backimagemenu"';
  
  if ($_GET["cmd"]!='imageback')
  
    $content.='style="visbility:hidden; display:none;'; 
	

  $content.='">';
  
  $userimages = getdir($userimages_dir);
 ...
 for ($i=0; $i<count($userimages); $i++) {
 	...
 	$content.='<div style="margin: 5px; float: left;">';
 	
 
     
  	$content.='<a onclick="apply(\'image\',\'0\',\''.$userimages[$i].'\'); return false;" href="#"><img width="200" src="'.$userimages_dir.'/'.$userimages[$i].'" alt="" /><br /><a onclick="deleteImageBackItem(\''.$userimages_dir.'/'.$userimages[$i].'\')" href="#"><img class="delete_sign" src="js/images/delete_sign.png"></a>'.$userimages[$i].'</a><br />';
