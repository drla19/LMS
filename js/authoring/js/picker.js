...
function openPicker(id, nextid, st, _onPick, _onCancel) {  
	if (_onPick) {
		onPick = _onPick;
	}
	if (_onCancel) {
...
/*
    if(browser == "Microsoft Internet Explorer") {
      ...
      iframe_document = iframe_window.document;
   	  ...
   	  if (iframe_document.getSelection) {
        var str = iframe_document.getSelection();
        if (window.RegExp) {
          var regstr = unescape("%20%20%20%20%20");
          var regexp = new RegExp(regstr, "g");
          str = str.replace(regexp, "");
       }
      } else if (iframe_document.selection && iframe_document.selection.createRange) {
       var range = iframe_document.selection.createRange();
       var str = range.text;
      }   
    ...
    */
    //Editor.execCommand('backcolor', false, 'green');
    //////////////
	...
	ObjNext = document.getElementById(nextid);
	
	orgColor = Obj.value;
	createLayer("picker",findPosX(ObjNext)+Obj.offsetWidth+20,findPosY(ObjNext));
}
...
function setClr(color){
	...
	browser = navigator.appName;
      	if (stav == 'textcolor') doForeCol();
	    if (stav == 'backcolor') doBackCol();
	    if (stav == 'course_backcolor') {
	    	$("#picker3").css('background-color',$("#color3").val());
	    	$("authoring-content .authoring-panes .authoring-pane #authoring-edit-area #container").css('background-color',$("#color3").val());
	    	
	    }
	    
	    if (stav == 'background_menu_color') {
	    	
	    	sd = $("#selected_div").val();
	    	
	    	$("#picker4").css('background-color',$("#color4").val());
	    	$("input#transparent").removeAttr("checked");
...
function colorTable(){
	...
	for(var j=0;j<3;j++){
		...
		for(var i=0;i<12;i++){
			...
			table += '<tr><td bgcolor="'+clr+'" class="cell_color" onMouseDown="colorize('+"'"+clr+"'"+')" onmouseover="showClr('+"'"+clr+"'"+')" onclick="setClr('+"'"+clr+"'"+')"></td></tr>';
		}
		table += '</table></td>';		
	}
	...
	for (var c = 0; c<6; c++) {
		...
		for (var j = 0; j<6; j++) {
			...
			for (var i = 0; i<6; i++) {
				...
				table += '<td bgcolor="'+clrhex+'" class="cell_color" onMouseDown="colorize('+"'"+clrhex+"'"+')" onmouseover="showClr('+"'"+clrhex+"'"+')" onclick="setClr('+"'"+clrhex+"'"+')"></td>';
			}
			table +="</tr>";
		}
		table +="</table>";
...
function colorize(color) {
  ...
  if ((status == 'editbackground') && (selected_div!='') && (selected_div!='0')) {
            
      var clr = $("#color5").val();
            
      $("#authoring_box"+selected_div).css("background-color",clr);
  ...
  else   if ((status == 'editbackground') && ((selected_div=='') || (selected_div=="0"))) {
      
      var clr = $("#color4").val();
      
      //$("#main_ul").css("background-color",clr);
      ...
      close_additional_menu(); 
       ...
       //ran = Editor.selection.createRange();
